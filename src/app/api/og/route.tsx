  ...
  if (eventType && headline) {
    return new ImageResponse(
      (
        ...
        >
          <div style={{ fontSize: 24, color: "#e55b3c", marginBottom: 12 }}>kingbid.lol · {eventType}</div>
          {room && <div style={{ fontSize: 22, color: "#8a7e76", marginBottom: 16 }}>{room}</div>}
          <div style={{ fontSize: 42, fontWeight: 800, color: "#1c1917", textAlign: "center" }}>{headline}</div>
        </div>
      ),
