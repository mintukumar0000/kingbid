...
export async function sendVerificationRequestAdminNotice(input: {
  id: string;
  email: string;
  name: string;
  message: string;
  ...
  const adminTo = process.env.CONTACT_EMAIL ?? "heyquixyz@gmail.com";
  const adminUrl = `${siteUrl()}/admin`;

  const lines = [
    `<p><strong>New Nepal campaign verification request</strong></p>`,
    `<p><strong>Email:</strong> ${input.email}</p>`,
    input.name ? `<p><strong>Name:</strong> ${input.name}</p>` : "",
    input.paymentPublicId ? `<p><strong>Transaction ID:</strong> ${input.paymentPublicId}</p>` : "",
    input.listingUrl ? `<p><strong>Listing:</strong> ${input.listingUrl}</p>` : "",
    input.message ? `<p><strong>Message:</strong><br/>${input.message.replace(/\n/g, "<br/>")}</p>` : "",
    `<p><strong>Request ID:</strong> ${input.id}</p>`,
    `<p><a href="${adminUrl}">Open admin →</a></p>`,
  ]
    .filter(Boolean)
    .join("\n");

  await send(adminTo, `Verification request from ${input.email}`, lines);
}

export async function sendVerificationRequestReceived(to: string, name?: string): Promise<void> {
  const greeting = name ? `Hi ${name},` : "Hi,";
  await send(
    to,
    "We received your Nepal campaign verification request",
    `<p>${greeting}</p>
     <p>Thanks for reaching out. We received your request for campaign verification proof.</p>
     <p>100% of eligible proceeds go to Nepal flood relief after Dodo payment settlement. We'll reply to this email with receipts or transfer details as soon as we can.</p>
     <p>You can also track live accounting anytime: <a href="${siteUrl()}/nepal-relief">${siteUrl()}/nepal-relief</a></p>`
  );
}
